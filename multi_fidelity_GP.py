...
for i in range(50):
    ...
    scaler.fit(X)
    X_train_stan_l = scaler.transform(X)
    X_train_stan_h = scaler.transform(X_train)
    X_train_mf, Y_train_mf = convert_xy_lists_to_arrays([X_train_stan_l, X_train_stan_h], [ctstar_wake_model[:,2][:,None], y[train_index,None] ])
